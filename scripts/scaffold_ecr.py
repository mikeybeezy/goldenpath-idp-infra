...
def update_ecr_catalog(
    catalog_path, name, registry_id, owner, risk, environment, requester, domain
):
    ...
    entry = "\n".join([f"  {name}:", "    metadata:"] + metadata_lines)
...
def main():
    ...
    parser.add_argument(
        "--risk", help="Risk profile (low, medium, high)", default="medium"
    )
    parser.add_argument(
        "--environment", help="Target environment (dev, prod)", default="dev"
    )
    parser.add_argument("--requester", help="Requesting user (e.g. daniel-deans)")
    parser.add_argument("--domain", help="Domain (e.g. delivery)")
    parser.add_argument(
        "--catalog-path", default="docs/20-contracts/resource-catalogs/ecr-catalog.yaml"
    )
    parser.add_argument(
        "--skip-catalog", action="store_true", help="Skip catalog update"
    )
    parser.add_argument(
    ...
    if interactive:
        print(f"--- ECR Scaffolding ({args.environment}) ---")
    app_name = args.app_name or get_input(
        "Application Name (e.g. goldenpath-wordpress-app)"
    )
    default_id = to_registry_id(app_name)
    app_id = args.app_id or get_input(
        "Governance ID (e.g. REGISTRY_{NAME})", default_id
    )
    validate_id(app_id)
    ...
    if interactive:
        requester = args.requester or get_optional(
            "Requesting User (e.g. daniel-deans)"
        )
