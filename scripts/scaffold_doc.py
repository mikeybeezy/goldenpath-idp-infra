...
def infer_id(path: Path, doc_type: str, explicit: str | None) -> str:
    ...
    if doc_type == "adr":
        ...
        raise ValueError(
            "ADR docs require an ID like ADR-0001 or filename ADR-0001-*.md"
        )
    if doc_type == "changelog":
        ...
        if match:
            return f"{match.group(1).upper()}-{match.group(2)}"
        raise ValueError(
            "Changelog docs require an ID like CL-0001 or filename CL-0001-*.md"
        )
...
def build_metadata(
    cfg: MetadataConfig, doc_type: str, doc_id: str, title: str, args
) -> dict:
...
def render_body(
    ...
    heading = (
        f"# {doc_id}: {title}" if doc_type in {"adr", "changelog"} else f"# {title}"
    )
    return f"{heading}\n\n## Summary\n\n- TODO\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create a new doc with compliant frontmatter."
    )
    ...
    parser.add_argument(
        ...
        dest="doc_type",
        default=None,
        help="Override doc type (adr, changelog, policy).",
    )
    parser.add_argument(
        "--id", dest="doc_id", default=None, help="Override document ID."
    )
    ...
    parser.add_argument(
        "--category", default=None, help="Override category (enum value)."
    )
    ...
    body = render_body(
        template_path, doc_type, doc_id, title, args, metadata.get("date", "")
    )
