...
test:
  runner: pytest
  command: pytest -q tests/scripts/test_script_0017.py
  evidence: declared
risk_profile:
    ...
    for adr_file in adr_files:
        ...
        entity = {
            ...
            "metadata": {
                ...
                "links": [
                    {"url": view_url, "title": "View on GitHub", "icon": "github"}
                ],
            },
            "spec": {
                "type": "documentation",
...
def create_location_files(adr_entities, changelog_entities, governance_entities):
    ...
    cl_location = {
        "apiVersion": "backstage.io/v1alpha1",
        "kind": "Location",
        "metadata": {"name": "all-changelogs", "description": "All Changelog Entries"},
        "spec": {
    ...
    gov_location = {
        ...
        "spec": {"targets": governance_entities},
    }
    dump_yaml(gov_location, output_dir / "governance-index.yaml")

    print("✅ Created location files with ALL entities")
