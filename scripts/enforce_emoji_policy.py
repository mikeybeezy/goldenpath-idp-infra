...
def scan_file(filepath, dry_run=False):
    is_authoritative = any(filepath.startswith(root) or root in filepath for root in NO_EMOJI_ROOTS)
    ...
    if violations:
        ...
        if not dry_run:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(new_content)
            print(f"   ✅ [FIXED] Removed non-compliant emojis.")
...
def main():
    ...
    parser.add_argument("--root", default=".", help="Directory to scan (ignored if files provided)")
