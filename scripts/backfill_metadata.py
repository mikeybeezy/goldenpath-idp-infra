...
test:
  runner: pytest
  command: pytest -q tests/scripts/test_script_0003.py
  evidence: declared
risk_profile:
    ...
    try:
        ...
        if "helm" in filepath.lower():
            ...
            version_match = re.search(
                r"(?:version|appVersion):\s*([\d\.]+)", content, re.IGNORECASE
            )
            if version_match:
...
def extract_dependencies(filepath):
    ...
    try:
        ...
        if "/apps/" in filepath:
            ...
            dependencies.extend(
                [f"image:{img}" for img in image_refs[:3]]
            )  # Limit to 3
...
def main():
    ...
    all_md_files = [
        f for f in all_md_files if not any(exc in f for exc in excluded_paths)
    ]
    ...
    if args.dry_run:
        ...
        print(
            "   3. Commit: git add . && git commit -m 'docs: complete metadata backfill'"
        )
