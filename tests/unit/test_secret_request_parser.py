...
from scripts.secret_request_parser import SecretRequest, parse_request, validate_enums, generate_tfvars, generate_externalsecret
...
class TestSecretRequestParser(unittest.TestCase):
    ...
    def test_parse_request_valid_camelCase(self):
        ...
        doc = {
            "id": "SEC-TEST",
            "metadata": {"name": "test-secret", "service": "test", "environment": "dev", "owner": "platform"},
            "spec": {
    ...
    def test_v1_policy_gate_high_risk_blocks_none_rotation(self):
        ...
        # Arrange
        req = SecretRequest(
            secret_id="SEC-0001", name="vault", service="core", environment="prod", owner="sec",
            secretType="api-key", riskTier="high", rotationClass="none", lifecycleStatus="active",
            namespace="core", k8sSecretName="vault-key", provider="aws-secrets-manager"
        )

        # Act & Assert
        with self.assertRaisesRegex(ValueError, "riskTier=high requires rotationClass != 'none'"):
    ...
    def test_generate_externalsecret_contains_correct_mapping(self):
        ...
        # Arrange
        req = SecretRequest(
            secret_id="SEC-007", name="db", service="app", environment="dev", owner="team",
            secretType="db-creds", riskTier="low", rotationClass="standard", lifecycleStatus="active",
            namespace="app", k8sSecretName="db-secret", provider="aws-secrets-manager"
        )
        ...
        self.assertEqual(es["spec"]["dataFrom"][0]["extract"]["key"], "goldenpath/dev/app/db")
