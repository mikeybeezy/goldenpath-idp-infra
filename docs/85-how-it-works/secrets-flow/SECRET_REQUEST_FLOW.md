...
- **Least-Privilege by Default**: Based on the `readPrincipals` and `writePrincipals` defined in the YAML, the parser automatically calculates and injects resource-level IAM policies, ensuring only authorized workloads can access the secret.
