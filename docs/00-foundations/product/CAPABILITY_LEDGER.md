...
- **Multi-Layer Deletion Protection**: Intentionally difficult deletion through AWS `deletion_protection`, Terraform `prevent_destroy` lifecycle, and the absence of any standard `rds-destroy` target—requiring explicit break-glass confirmation (`rds-destroy-break-glass`) for destruction.
