...
|098|P2|**🔴 HV/HQ**|GitOps|Hard Guard: ArgoCD tier labels + ApplicationSet for tier gating|platform|Open|M|Q2|Add `goldenpath.idp/tier` labels to all ArgoCD Application manifests; create ApplicationSet with tier selector|ADR-0180. Currently maxTier is computed but apps deploy based on chart availability, not tier enforcement. Hard guard ensures tier 2+ apps cannot sync when `maxTier < 2`.|
|099|P2|**🔴 HV/HQ**|Security|Hard Guard: IAM policy scoping for ephemeral DNS writes|platform|Open|M|Q2|Create IAM policy that restricts `route53:ChangeResourceRecordSets` to `*.b-{buildId}.{env}.*` for ephemeral IRSA roles|ADR-0178. domainFilters is config-based; nothing prevents ExternalDNS from writing outside scope if misconfigured. IAM policy is architecture-driven enforcement.|
