...
# =============================================================================
# TEARDOWN V5 - Enhanced EKS Cluster Teardown Runner with DNS Cleanup
# =============================================================================
#
# Version: 5.0.0
# Purpose: Reliable teardown of EKS clusters with proper resource ordering
#          including ExternalDNS cleanup for Route53 record hygiene
#
# Key improvements over v4:
#   - ExternalDNS Argo application cleanup before ingress/LB teardown
#   - Explicit DNS record ownership sequencing (ExternalDNS -> Kong -> LB)
#   - Keeps v4 safety defaults for RDS/secrets cleanup
#
# Key improvements over v3 (inherited):
#   - Secrets Manager cleanup by BuildId tag and name pattern
#   - Handles secrets in recovery window (force delete or restore+delete)
#   - Audit logging for secrets deletion
#   - Supports both ephemeral (BuildId-tagged) and persistent (pattern-matched) secrets
#
# Key improvements in v3.1.0 (inherited):
...
#
# Key improvements in v3.0.0 (inherited):
...
#
# Usage:
#   bootstrap/60_tear_down_clean_up/goldenpath-idp-teardown-v5.sh <cluster-name> <region>
#
# Required:
...
#   SUSPEND_EXTERNAL_DNS_APP=true|false (default true)
#   DELETE_EXTERNAL_DNS_APP=true|false (default true)
#   EXTERNAL_DNS_DELETE_WAIT_SECONDS=<seconds> (default 10, set 0 to disable)
#   EXTERNAL_DNS_APP_NAMESPACE=<namespace> (default argocd)
#   EXTERNAL_DNS_APP_NAME=<name> (default <env>-external-dns)
...
# Extract environment from cluster name (e.g., goldenpath-dev-eks -> dev)
_env_from_cluster=$(echo "${cluster_name}" | sed -n 's/.*goldenpath-\([^-]*\).*/\1/p')
...
# ExternalDNS ArgoCD application settings
SUSPEND_EXTERNAL_DNS_APP="${SUSPEND_EXTERNAL_DNS_APP:-true}"
DELETE_EXTERNAL_DNS_APP="${DELETE_EXTERNAL_DNS_APP:-true}"
EXTERNAL_DNS_DELETE_WAIT_SECONDS="${EXTERNAL_DNS_DELETE_WAIT_SECONDS:-10}"
EXTERNAL_DNS_APP_NAMESPACE="${EXTERNAL_DNS_APP_NAMESPACE:-argocd}"
EXTERNAL_DNS_APP_NAME="${EXTERNAL_DNS_APP_NAME:-${_env_from_cluster:-dev}-external-dns}"
...
SECRETS_FORCE_DELETE="${SECRETS_FORCE_DELETE:-true}"
SECRETS_NAME_PATTERN="${SECRETS_NAME_PATTERN:-goldenpath/${_env_from_cluster:-dev}/}"
...
if [[ "${DRY_RUN}" == "true" ]]; then
  ...
  DELETE_EXTERNAL_DNS_APP="false"
  SUSPEND_EXTERNAL_DNS_APP="false"
...
# =============================================================================
# EXTERNALDNS ARGOCD FUNCTIONS
# =============================================================================

suspend_external_dns_application() {
  if [[ "${SUSPEND_EXTERNAL_DNS_APP}" != "true" ]]; then
    return 0
  fi

  log_step "SUSPEND_EXTERNAL_DNS" "Suspending Argo Application ${EXTERNAL_DNS_APP_NAMESPACE}/${EXTERNAL_DNS_APP_NAME}..."
  ...
  if ! kubectl get ns "${EXTERNAL_DNS_APP_NAMESPACE}" >/dev/null 2>&1; then
    log_info "Namespace ${EXTERNAL_DNS_APP_NAMESPACE} not found; skipping."
    return 0
  fi

  if ! kubectl -n "${EXTERNAL_DNS_APP_NAMESPACE}" get application "${EXTERNAL_DNS_APP_NAME}" >/dev/null 2>&1; then
    log_info "Application ${EXTERNAL_DNS_APP_NAMESPACE}/${EXTERNAL_DNS_APP_NAME} not found; skipping."
    return 0
  fi

  kubectl -n "${EXTERNAL_DNS_APP_NAMESPACE}" patch application "${EXTERNAL_DNS_APP_NAME}" \
    --type merge \
    -p '{"spec":{"syncPolicy":null}}' >/dev/null 2>&1 || true
  log_info "Suspended ExternalDNS Argo Application sync policy."
}

delete_external_dns_application() {
  if [[ "${SUSPEND_EXTERNAL_DNS_APP}" == "true" ]]; then
    suspend_external_dns_application
  fi

  if [[ "${DELETE_EXTERNAL_DNS_APP}" != "true" ]]; then
    log_info "Skipping ExternalDNS Argo application deletion (DELETE_EXTERNAL_DNS_APP=false)."
    return 0
  fi

  log_step "DELETE_EXTERNAL_DNS" "Deleting Argo Application ${EXTERNAL_DNS_APP_NAMESPACE}/${EXTERNAL_DNS_APP_NAME}..."

  if [[ -z "${EXTERNAL_DNS_APP_NAMESPACE}" || -z "${EXTERNAL_DNS_APP_NAME}" ]]; then
    log_info "Skipping ExternalDNS Argo application deletion (namespace/name not set)."
    return 0
  fi
  ...
  if ! kubectl --request-timeout="${KUBECTL_REQUEST_TIMEOUT}" get ns "${EXTERNAL_DNS_APP_NAMESPACE}" >/dev/null 2>&1; then
    log_info "Namespace ${EXTERNAL_DNS_APP_NAMESPACE} not found; skipping ExternalDNS deletion."
    return 0
  fi

  if ! kubectl --request-timeout="${KUBECTL_REQUEST_TIMEOUT}" -n "${EXTERNAL_DNS_APP_NAMESPACE}" get application "${EXTERNAL_DNS_APP_NAME}" >/dev/null 2>&1; then
    log_info "ExternalDNS Argo application ${EXTERNAL_DNS_APP_NAMESPACE}/${EXTERNAL_DNS_APP_NAME} not found; skipping deletion."
    return 0
  fi

  if ! kubectl --request-timeout="${KUBECTL_REQUEST_TIMEOUT}" -n "${EXTERNAL_DNS_APP_NAMESPACE}" delete application "${EXTERNAL_DNS_APP_NAME}" --wait=false --ignore-not-found; then
    log_warn "Failed to delete ExternalDNS Argo Application ${EXTERNAL_DNS_APP_NAMESPACE}/${EXTERNAL_DNS_APP_NAME}."
  else
    log_info "Deleted ExternalDNS Argo Application ${EXTERNAL_DNS_APP_NAMESPACE}/${EXTERNAL_DNS_APP_NAME}."
    if [[ "${EXTERNAL_DNS_DELETE_WAIT_SECONDS}" =~ ^[0-9]+$ ]] && [[ "${EXTERNAL_DNS_DELETE_WAIT_SECONDS}" -gt 0 ]]; then
      log_info "Waiting ${EXTERNAL_DNS_DELETE_WAIT_SECONDS}s to allow DNS cleanup."
      sleep "${EXTERNAL_DNS_DELETE_WAIT_SECONDS}"
    fi
  fi
}
...
if [[ "${cluster_exists}" == "false" ]]; then
  ...
  if [[ -z "${nodegroups}" ]]; then
    ...
    drain_script="${repo_root}/bootstrap/60_tear_down_clean_up/drain-nodegroup.sh"
    if [[ ! -f "${drain_script}" ]]; then
      log_warn "Drain script not found at ${drain_script}; skipping nodegroup drain."
    else
      for ng in ${nodegroups}; do
        log_step "DRAIN_NODEGROUP" "Draining nodegroup ${ng}..."
        RELAX_PDB="${RELAX_PDB:-true}" bash "${drain_script}" "${ng}" || \
...
# =============================================================================
# STAGE 6: DELETE SECRETS MANAGER SECRETS
# =============================================================================

stage_banner "6" "DELETE SECRETS"

if [[ "${DELETE_SECRETS}" == "true" ]]; then
  log_step "SECRETS_CLEANUP" "Cleaning up Secrets Manager secrets..."
  secrets_deleted=0
  secrets_failed=0

  # Method 1: Find secrets by BuildId tag (if BUILD_ID is set)
  if [[ -n "${BUILD_ID:-}" ]]; then
    log_info "Searching for secrets with BuildId=${BUILD_ID}..."
    secret_arns=$(aws resourcegroupstaggingapi get-resources \
      --tag-filters "Key=BuildId,Values=${BUILD_ID}" \
    ...
    for arn in ${secret_arns}; do
      secret_name="${arn##*:secret:}"
      # Remove version suffix if present (e.g., secret-name-AbCdEf)
      secret_name="${secret_name%%-[A-Za-z0-9][A-Za-z0-9][A-Za-z0-9][A-Za-z0-9][A-Za-z0-9][A-Za-z0-9]}"
      log_step "DELETE_SECRET" "Deleting secret: ${secret_name}"

      delete_args="--secret-id ${arn} --region ${region}"
      if [[ "${SECRETS_FORCE_DELETE}" == "true" ]]; then
        delete_args="${delete_args} --force-delete-without-recovery"
      fi

      if aws secretsmanager delete-secret ${delete_args} 2>/dev/null; then
        log_info "  Deleted: ${secret_name}"
        ((secrets_deleted++)) || true
      else
        log_warn "  Failed to delete: ${secret_name}"
        ((secrets_failed++)) || true
      fi
    done
  fi

  # Method 2: Find secrets by name pattern (fallback/additional)
  if [[ -n "${SECRETS_NAME_PATTERN}" ]]; then
    log_info "Searching for secrets matching pattern: ${SECRETS_NAME_PATTERN}*"
    pattern_secrets=$(aws secretsmanager list-secrets \
      --region "${region}" \
      --query "SecretList[?starts_with(Name, '${SECRETS_NAME_PATTERN}')].ARN" \
      --output text 2>/dev/null || true)

    for arn in ${pattern_secrets}; do
      # Skip if already processed via BuildId tag
      if [[ -n "${BUILD_ID:-}" ]]; then
        secret_tags=$(aws secretsmanager describe-secret --secret-id "${arn}" --region "${region}" \
          --query "Tags[?Key=='BuildId'].Value" --output text 2>/dev/null || true)
        if [[ "${secret_tags}" == "${BUILD_ID}" ]]; then
      ...
      secret_name="${arn##*:secret:}"
      secret_name="${secret_name%%-[A-Za-z0-9][A-Za-z0-9][A-Za-z0-9][A-Za-z0-9][A-Za-z0-9][A-Za-z0-9]}"
      log_step "DELETE_SECRET" "Deleting secret (pattern match): ${secret_name}"
  ...
  log_info "Secrets cleanup complete: ${secrets_deleted} deleted, ${secrets_failed} failed."
else
  log_info "Skipping secrets cleanup (DELETE_SECRETS=${DELETE_SECRETS})."
fi

stage_done "6" "DELETE SECRETS"
...
stage_done "7" "TERRAFORM DESTROY"

# =============================================================================
# STAGE 8: ORPHAN CLEANUP
# =============================================================================

stage_banner "8" "ORPHAN CLEANUP"
...
stage_done "8" "ORPHAN CLEANUP"

# =============================================================================
# STAGE 9: TEARDOWN COMPLETE
# =============================================================================

stage_banner "9" "TEARDOWN COMPLETE"

log_info "============================================================================="
log_info "TEARDOWN V5 COMPLETED SUCCESSFULLY"
log_info "============================================================================="
log_info "Cluster: ${cluster_name}"
log_info "Region: ${region}"
...
stage_done "9" "TEARDOWN COMPLETE"
